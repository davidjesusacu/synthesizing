    ...
    for o in params:
        ...
        for i in xrange(o['iter_n']):
            ...
            def prob(input):
                ...
                generated = generator.forward(feat=input)
                x0 = generated[gen_out_layer]  # 256x256

                # Crop from 256x256 to 227x227
                cropped_x0 = x0.copy()[:, :, topleft[0]:topleft[0] + image_size[0],
                ...
                grad_norm_net, x, act = make_step_net(net=net, end=layer, unit=unit, image=cropped_x0, xy=xy,
                                                      step_size=step_size)
                ...
                updated_x0 = x0.copy()
                updated_x0[:, :, topleft[0]:topleft[0] + image_size[0],
                topleft[1]:topleft[1] + image_size[1]] = x.copy()
                ...
                grad_norm_generator, updated_code = make_step_generator(net=generator, x=updated_x0, x0=x0,
                                                                        start=gen_in_layer, end=gen_out_layer,
                                                                        step_size=step_size)
                return best_xx, grad_norm_generator, updated_code, act
                ...
                # noiseON = 0
                # updated_code += noiseON * np.sqrt(2 * step_size) * np.random.normal(0, 1, updated_code.shape)

                # here my x is act
                # my gradient is updated coded.

                # updaX, upcode = prob(src.data[:])

            best_xx, grad_norm_generator, best_act, updated_code = MH(src.data[:], prob, step_size)

            # Clipping expected new code
            if clip:
            ...
            # here updated code has to be the expectation of the src_data.
            # Update code
            ...
            #Perfect optimization in such high-dim doesnt happen
            # Stop if grad is 0
...
def write_label(filename, act):
    ...
    subprocess.call(
        ["convert %s -append -gravity Center -pointsize %s label:\"%.2f\" -bordercolor white -border 0x0 -append %s" %
         (filename, 30, act, filename)], shell=True)


def main():
    # sys.argv[1:] = ['--act_layer', 'fc8', '--opt_layer', 'fc6', '--unit', '643', '--xy', '0',
    #                 '--n_iters',
    #                 '200',
    #                 '--start_lr', '8.0', '--end_lr', '1e-10', '--L2', '0.99', '--seed', '0', '--clip', '0', '--bound',
    #                 'act_range/3x/fc6.txt', '--debug', '1', '--output_dir', 'output', '--init_file', 'None']
    ...
    parser.add_argument('--bound', metavar='b', type=str, default="",
                        help='The file to an array that is the upper bound for activation range')
    parser.add_argument('--output_dir', metavar='b', type=str, default=".", help='Output directory for saving results')
    parser.add_argument('--net_weights', metavar='b', type=str, default=settings.net_weights,
                        help='Weights of the net being visualized')
    parser.add_argument('--net_definition', metavar='b', type=str, default=settings.net_definition,
